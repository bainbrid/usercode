...
TH1D* rebinNew( TFile* file, const char* name, int multi, int nbins = -1, double xlow = -1., double xhigh = -1. ) {
  ...
  if ( multi == 0 ) {
    ...
    if (temp) { histo = (TH1D*)temp->Clone(); } 
    else { std::cout << "Unable to retrieve histo!" << std::endl; }
  } else if ( multi < 0 ) {
    for ( uint ii = abs(multi); ii < 9; ++ii ) {
      std::stringstream tmp; tmp << name << "_" << ii;
      if ( int(ii) == abs(multi) ) { 
  ...
  if (debug) {
    if ( histo ) {
      TH1D* his = histo;
      std::stringstream ss;
      ss << "ORIG HISTO: " << file->GetName() << "/" << name << " multi: " << (multi<0?">=":"==") << abs(multi) 
	 << std::endl
	 << " underflow: " << his->GetBinContent(0)
	 << std::endl
	 << " underflow_err: " << his->GetBinError(0)
  ...
  // Create new histogram without last bin
  TH1D* input = new TH1D( TString(histo->GetName())+"_DropLastBin", "", 
			  histo->GetXaxis()->GetNbins()-1, 
			  histo->GetXaxis()->GetXmin(), 
			  histo->GetXaxis()->GetXmax() - histo->GetBinWidth(histo->GetXaxis()->GetNbins()) );
  input->Sumw2();

  // Populate new histogram
  for ( int bin = 1; bin <= input->GetNbinsX(); ++bin ) {
    Double_t centre = input->GetBinLowEdge(bin) + input->GetBinWidth(bin)/2.;
    int find_bin = histo->FindBin(centre);
    input->SetBinContent( bin, histo->GetBinContent( find_bin ) );
    input->SetBinError( bin, histo->GetBinError( find_bin ) );
  }
  input->SetEntries( histo->GetEntries() );
...
// 	    if ( lumi > 0. ) {
// 	      if ( ifile != data_file ) { 
// 		numerator->Scale(lumi/100.);
// 		denominator->Scale(lumi/100.);
// 	      }
// 	    } else {
...
// 		  double upper = weights[iht] * (lumi>0.?lumi:lumis[ifile]) / 100.;
// 		  double lower = denom[ifile][imulti][iat][ipt][iht];
// 		  errh[ifile][imulti][iat][ipt][iht] = lower > 0. ? upper / lower : 0.;
// 		}
// 	      }

//  	      // Account for different MC weights
// 	      if ( (int)ifile != data_file ) { 
// 		double el = errl[ifile][imulti][iat][ipt][iht];
// 		if ( el > 0. ) { errl[ifile][imulti][iat][ipt][iht] = sqrt(weights[iht]/100.) * el; }
// 		double eh = errh[ifile][imulti][iat][ipt][iht];
// 		if ( eh > 0. ) { errh[ifile][imulti][iat][ipt][iht] = sqrt(weights[iht]/100.) * eh; }
// 	      }
