...
bool EffPlots::Plots( Event::Data& ev ) {
  ...
  std::stringstream ss;
  ss << "[EffPlots::Plots]" << std::endl;
  if ( ev.JD_CommonJets().accepted.size() == 2 ) { ss << " DIJET"; }
  else if ( ev.JD_CommonJets().accepted.size() > 2 ) { ss << " NJETS"; }
  else { ss << "MONOJET"; }
  ss << std::endl;
  ss << " Number of common jets: " << ev.JD_CommonJets().accepted.size() << std::endl;
  if ( ev.JD_CommonJets().accepted.size() != ev.CommonObjects().size() ) {
    ss << " PROBLEM!! jet != common: " << ev.JD_CommonJets().accepted.size() 
       << " " << ev.CommonObjects().size()
       << std::endl;
  }
  ...
  double HT = 0.;
    for(int i = 0 ; i < ev.JD_CommonJets().accepted.size(); i++){
      ss << " i: " << i << " ET: " << ev.JD_CommonJets().accepted[i]->Et()
	 << " eta: " << ev.JD_CommonJets().accepted[i]->Eta() << std::endl;
    ...
    ss << " HT: " << HT << " MHT: " << MHT << " MHT/HT: " << MHT/HT
       << std::endl
       << " aT: " << ev.HadronicAlphaT()
       << " bT: " << sqrt( 1. / ( 4. * ( 1. - ( (MHT/HT) * (MHT/HT) ) ) ) )
       << std::endl;
    
    //if ( verbose_ && ev.HadronicAlphaT() > 0.6 ) { std::cout << ss.str(); }
