...
//
        ...
        for(int idx = 0; iEcalB!=jEcalB;++iEcalB,++idx){
          if( ev.rechitCaloSeverityLevelEb()->at(idx) > 1 ) sumRecHitPt += (*iEcalB).Pt();
        }
        // cout << " Starting EE " << endl;
        for(int idx = 0; iEcalE!=jEcalE;++iEcalE,++idx){
          if( ev.rechitCaloSeverityLevelEe()->at(idx) > 1 ) sumRecHitPt += (*iEcalE).Pt();
        }
        // cout << " Starting HEHB " << endl;
        for(int idx = 0; iHbHe!=jHbHe;++iHbHe,++idx){
          if( ev.rechitCaloSeverityLevelHbhe()->at(idx) > 9 ) sumRecHitPt += (*iHbHe).Pt();
        }
        // cout << " Starting HF " << endl;
        for(int idx = 0; iHF!=jHF;++iHF,++idx){
          if( ev.rechitCaloSeverityLevelHf()->at(idx) > 9 ) sumRecHitPt += (*iHF).Pt();
        }
