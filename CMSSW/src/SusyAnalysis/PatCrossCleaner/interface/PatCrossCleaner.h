...
// $Id: PatCrossCleaner.h,v 1.14 2009/02/10 13:26:14 georgia Exp $
//
//
...
class PatCrossCleaner : public edm::EDProducer {
  ...
  template <class Object>  CrossCleanerMETCorrection correctObject(Object&, CrossCleanerModifier&, const edm::EventSetup&);
  CrossCleanerMETCorrection correctObject(pat::Electron & , CrossCleanerModifier &, const edm::EventSetup&);
  CrossCleanerMETCorrection correctObject(pat::Jet & , CrossCleanerModifier &, const edm::EventSetup& );
...
CrossCleanerMETCorrection PatCrossCleaner::correctObject(Object & o, CrossCleanerModifier & modifier, const edm::EventSetup&)
{return CrossCleanerMETCorrection();}

CrossCleanerMETCorrection PatCrossCleaner::correctObject(pat::Jet & o, CrossCleanerModifier & modifier, const edm::EventSetup& es){
  CrossCleanerMETCorrection metCorr(o.correctedJet("RAW").px() - o.px(), o.correctedJet("RAW").py() - o.py());
  if (modifier.keepKeyObj){
    ...
    if (o.energy()!=0){
      ...
      pat::Jet::LorentzVector newP4( (oNoCorr.px()-modifier.modEnergy.X()), (oNoCorr.py()-modifier.modEnergy.Y()), (oNoCorr.pz()-modifier.modEnergy.Z()), (oNoCorr.energy()- theSharedE) );
      o.setP4( newP4 ); // need to redo jet corrections now!
            	  
      LogDebug("PatCrossCleaner")<< "changing jet uncorrected energy from: "<<oNoCorr.energy()<<" to: "<<  (oNoCorr.energy()- theSharedE)
	//				 << "\n the scaling factor is: "<< k
				 << "\n the modifier energy is: "<< theSharedE
				 << "\n modifier id: "<<modifier.object->pdgId();
      
      /*
      double oldE = oNoCorr.energy();
      double newE = oNoCorr.energy() + modifier.modEnergy;
      double k = newE/oNoCorr.energy();
      pat::Jet::LorentzVector newP4(k*oNoCorr.px(), k*oNoCorr.py(), k*oNoCorr.pz(), newE );
      o.setP4( newP4 ); // need to redo jet corrections now!
    	  
      LogDebug("PatCrossCleaner")<< "changing jet uncorrected energy from: "<<oldE<<" to: "<< newE
				 << "\n the scaling factor is: "<< k
				 << "\n the modifier energy is: "<<modifier.modEnergy
				 << "\n modifier id: "<<modifier.object->pdgId();
      */
      ...
      LogDebug("PatCrossCleaner") << "L3 jet correction factor for changed p4 is " << fabs(aJetCorr.correction(pat::JetCorrFactors::L3))
  ...
  metCorr += CrossCleanerMETCorrection(o.px() - o.correctedJet("RAW").px(), o.py() - o.correctedJet("RAW").py());
  return metCorr;
}

// correct an electron due to a modifier.
CrossCleanerMETCorrection PatCrossCleaner::correctObject(pat::Electron & o, CrossCleanerModifier & modifier, const edm::EventSetup&){
  CrossCleanerMETCorrection metCorr;
  //so far, no correction to MET, if an object values are corrected.
				 ...
				 edm::Event & iEvent, const edm::EventSetup& es, std::string Label)
{
  ...
  for (uint i = 0; i < initial.size(); ++i){
    ...
    if (action != assMap.end()){
      LogDebug("PatCrossCleaner")<<Label<<" "
				 <<" component keep: "<<action->second.keepFromModifiers();

      if (action->second.keepFromModifiers()){
	keepMe=true;
	for (uint m=0;m!=action->second.modifiers.size();++m){
	  
	  CrossCleanerMETCorrection thisMETcorr =correctObject(newObject ,action->second.modifiers[m], es);
	  if (newObject.energy() < 0.0){
	    keepMe=false;
	    continue;
	  }
	  else{
