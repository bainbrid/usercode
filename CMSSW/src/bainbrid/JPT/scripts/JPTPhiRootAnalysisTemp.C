...
//by  b_branchname->GetEntry(ientry); //read only this branch
   ...
   const Int_t nby = nbins+1;
   const Float_t ybins[nby]={0.00,0.05.,0.10.,0.15.,0.20.,0.25.,0.30.,0.35.,0.40.,0.45.,0.50.};
   TH2F* h2DeltaR = new TH2F("h2DeltaR", "h2DeltaR", nbins, xbins, nbins, ybins );
...
//       }
	...
	if(EtGen1 >= xbins[ih] && EtGen1 < xbins[ih+1] ) {
	  h2DeltaR->Fill( EtGen1,deltaR(EtaJPT1,EtaGen1,PhiJPT1,PhiGen1) );
	  if ( DRMAXgjet1 < 0.30
	...
	if(EtGen2 >= xbins[ih] && EtGen2 < xbins[ih+1] ) {
	  h2DeltaR->Fill( deltaR(EtaJPT2,EtaGen2,PhiJPT2,PhiGen2) );
	  if ( DRMAXgjet2 < 0.30
   ...
   */
   ...
   TCanvas* tmp1 = new TCanvas("X11","Y11",1);
   tdrStyle->SetOptStat(1111);
   //tdrStyle->SetOptStat("mr");
   h2DeltaR->Draw();
   tmp1->SaveAs("DeltaR_2d.png");
