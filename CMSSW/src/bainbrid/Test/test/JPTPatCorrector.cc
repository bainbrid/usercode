...
// -----------------------------------------------------------------------------
//
JPTPatCorrector::JPTPatCorrector( const edm::ParameterSet& pset ) 
  : JPTCorrector(pset),
...
// -----------------------------------------------------------------------------
//
JPTPatCorrector::~JPTPatCorrector() {;}

// -----------------------------------------------------------------------------
//
bool JPTPatCorrector::jtaOnTheFly( const reco::Jet& fJet,
  ...
  if ( !allowOnTheFly_ ) {
    ...
    edm::LogWarning("JPTPatCorrector") 
      << "[JPTPatCorrector::" << __func__ << "]"
...
// -----------------------------------------------------------------------------
//
void JPTPatCorrector::matchTracks( const JetTracks& jet_tracks, 
				   const edm::Event& event, 
				   MatchedTracks& pions, 
				   MatchedTracks& muons,
  ...
  if ( verbose_ && edm::isDebugEnabled() ) {
    std::stringstream ss;
    ss << "[JPTPatCorrector::" << __func__ << "] Number of tracks:" << std::endl 
    ...
    LogTrace("JPTPatCorrector") << ss.str();
  }
...
// -----------------------------------------------------------------------------
//
bool JPTPatCorrector::getMuons( const edm::Event& event, edm::Handle<PatMuons>& pat_muons ) const {
...
// -----------------------------------------------------------------------------
//
bool JPTPatCorrector::getElectrons( const edm::Event& event, edm::Handle<PatElectrons>& pat_elecs ) const {
...
// -----------------------------------------------------------------------------
//
bool JPTPatCorrector::matchMuons( reco::TrackRefVector::const_iterator itrk, 
...
// -----------------------------------------------------------------------------
//
bool JPTPatCorrector::matchElectrons( reco::TrackRefVector::const_iterator itrk, 
