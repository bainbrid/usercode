...
// $Id: PatCrossCleaner.h,v 1.13 2009/02/04 17:00:46 bmura Exp $
//
//
...
class PatCrossCleaner : public edm::EDProducer {
  ...
  template <class Object> void putObjects( CrossCleanerResult&, 
					   const edm::View<Object>& initial,
					   edm::Event&, 
...
void PatCrossCleaner::correctObject(pat::Electron & o, CrossCleanerModifier & modifier, const edm::EventSetup&){
  if (modifier.keepKeyObj){
    if (o.energy()!=0){
      double oldEt = o.et();
      double newE = o.energy() + modifier.modEnergy;
...
template <class Object>
void PatCrossCleaner::putObjects( CrossCleanerResult& result, 
				  const edm::View<Object> & initial,
				  edm::Event& iEvent, 
				  ...
				  std::string Label )
{
  ...
  cleanObj->reserve(initial.size());
  droppedObj->reserve(initial.size());
